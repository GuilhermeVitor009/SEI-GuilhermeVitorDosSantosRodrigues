...
vel_vert = 0
vel_horiz = 0
x = np.array([w1, w2, pos[0], pos[1], v1, v2, angulo * np.pi/180., vel_ang*np.pi/180.])
#Dados do drone
m = 0.25
g = 9.79
P = m*g
...
def rotacionar(img, angulo):
    ...
    posr = (pos[0] - drone_rodado.get_width()/10, pos[1] - drone_rodado.get_height()/4)
    return drone_rodado

def auto_estabilizar(estavel):
    comando = pygame.key.get_pressed()
    if comando[pygame.K_g]:
        estavel = True
    if comando[pygame.K_e]:
        estavel = False
...
def waypoints(pos, F, x, dt, angulo, y_direction):
    xpos = [0, 0]
    wp = [[0., 10], [15., 25], [-30, 70]]
    pi = [500, 400]
...
# Eventos de controle de drone
    ...
    drone_rodado = rotacionar(img_drone, angulo)# Atua na estabilização da rotação do drone em movimento
    pos = velocidade_resultante(pos, F, P, m, estavel, angulo, step, vel_vert, vel_horiz, dt, y_direction) #Movimenta o drone 
